...
------------------------------------------
function ulx.seepsay(calling_ply)
...
local seepsay = ulx.command(CATEGORY_NAME, "ulx seepsay", ulx.seepsay, "!seepsay", true)
seepsay:defaultAccess( ULib.ACCESS_ADMIN )
seepsay:help("Lets the user see all private messaging going on")
...
hook.Add("ULibPostTranslatedCommand", "RelayPsay", function(ply, commandname, args)
	...
	if args[3] ~= nil then
		...
		ulx.fancyLog(toggledon, "[SeePsay] #P to #P: " .. args[3], args[1], args[2])	
	end
end)
